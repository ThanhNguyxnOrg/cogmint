...
const { supportedLocales, localeLabels, setLocale, syncFromSettings, t } = useI18n()
const {
  skillImports,
  agentImports,
...
const localeModel = ref<string>('en')

watch(settings, (val) => {
  localeModel.value = typeof val?.locale === 'string' ? val.locale : 'en'
}, { immediate: true })
...
const localeOptions = computed(() => {
  return supportedLocales.map(code => ({
    ...
    description: code === 'en' ? 'Default' : 'Preview',
  }))
})

async function saveLanguage() {
  const next = localeModel.value || 'en'
  setLocale(next)
...
<template>
  <div>
    ...
    <div v-else-if="viewMode === 'structured'" class="px-6 py-4 space-y-6">
      ...
      >
        <h3 class="text-section-title">{{ t('settings.general') }}</h3>

        <div class="space-y-4">
          <div class="field-group">
            <label class="field-label">{{ t('settings.locale') }}</label>
            <USelectDropdown v-model="localeModel" :options="localeOptions" />
            <span class="field-hint">{{ t('settings.localeDescription') }}</span>
          ...
          <div class="flex items-center justify-between">
            <div>
              <div class="text-[13px] font-medium">{{ t('settings.alwaysThinking') }}</div>
              <div class="text-[12px] mt-0.5 text-label">
                {{ t('settings.alwaysThinkingDescription') }}
              </div>
            </div>
      ...
      <div
        class="rounded-xl p-5 space-y-4 bg-card"
      >
        <h3 class="text-section-title">{{ t('settings.statusLine') }}</h3>
        <p class="text-[12px] text-meta">
          {{ t('settings.statusLineDescription') }}
        </p>
      ...
      <div class="rounded-xl p-5 space-y-4 bg-card">
        <div class="flex items-center justify-between">
          <h3 class="text-section-title">{{ t('settings.githubImports') }}</h3>
          <UButton
      ...
      >
        <div class="flex items-center justify-between">
          <h3 class="text-section-title">{{ t('settings.automations') }}</h3>
